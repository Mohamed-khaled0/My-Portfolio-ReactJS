...
const ProjectDetails = () => {
  ...
  return (
    <div style={{ maxWidth: 800, margin: '40px auto', background: '#fff', borderRadius: 12, boxShadow: '0 2px 12px rgba(36,99,235,0.08)', padding: 32 }}>
      ...
      <h1 style={{ color: '#2563eb', marginBottom: 12 }}>{project.projectTitle}</h1>
      <h3 style={{ color: '#555', marginBottom: 18 }}>{project.projectSubtitle}</h3>
      <img src={project.imgPath} alt={project.projectTitle} style={{ width: '100%', maxWidth: 500, borderRadius: 8, marginBottom: 24 }} />
      ...
      <div style={{ marginTop: 24, display: 'flex', gap: 16 }}>
        {project.liveDemo && (
          <a href={project.liveDemo} target="_blank" rel="noopener noreferrer" style={{ background: 'linear-gradient(90deg, #2563eb 0%, #1d4ed8 100%)', color: '#fff', borderRadius: 999, padding: '10px 22px', textDecoration: 'none', fontWeight: 500 }}>Live Demo</a>
        )}
        {project.githubLink && (
          <a href={project.githubLink} target="_blank" rel="noopener noreferrer" style={{ background: '#222', color: '#fff', borderRadius: 999, padding: '10px 22px', textDecoration: 'none', fontWeight: 500 }}>GitHub</a>
        )}
        {project.githubRepos && (
